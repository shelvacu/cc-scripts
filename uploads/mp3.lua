...
local function messagePackImpl(val, output)
  local ty = type(val)
  if ty == "number" then
    if isInt(val) then
      if val >= 0 and val < 2^7 then
        table.insert(output, string.char(val))
      elseif val >= 0 and val < 2^8 then
        table.insert(output, string.char(0xcc, val))
      elseif val >= 0 and val < 2^16 then
        table.insert(output, string.char(0xcd, bit.blogic_rshift(val, 8), bit.band(val, 0xff)))
      ...
      elseif val >= -2^5 then
        table.insert(output, string.char(0x100 + val))
      elseif val >= -2^7 then
        table.insert(output, string.char(0xd0, 0x100 + val))
      elseif val >= -2^15 then
        local uns = 2^16 + val
        table.insert(output, string.char(0xd1, bit.blogic_rshift(uns, 8), bit.band(uns, 0xff)))
  ...
  elseif ty == "nil" then
    table.insert(output, string.char(0xc0))
  elseif ty == "boolean" then
    if val then
      table.insert(output, string.char(0xc3))
    else
      table.insert(output, string.char(0xc2))
    end
  ...
  elseif ty == "table" then
    if val[1] then --assume array-like table
      ...
      local prefix
      local res = {} 
      table.insert(output, "") -- hold a spot for the prefix to go
      local prefixSpot = #output
      for k,v in pairs(val) do
        messagePackImpl(k, output)
        messagePackImpl(v, output)
        if math.fmod(iters, 1024) == 0 then os.sleep() end
        len = len + 1
      end
      if len <= 15 then
